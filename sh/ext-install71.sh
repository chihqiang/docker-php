...
apt install -y --no-install-recommends  coreutils cron autoconf make gcc wget
# php default ext
...
if [ -n "${HIREDIS_VERSION+x}" ]; then
    cd /tmp && wget https://github.com/redis/hiredis/archive/v${HIREDIS_VERSION}.tar.gz -O hiredis.tar.gz \
    && mkdir -p hiredis \
...
if [ -n "${SWOOLE_VERSION+x}" ]; then
    ...
    apt-get install -y --no-install-recommends openssl libssl-dev libcurl4-openssl-dev libnghttp2-dev 
